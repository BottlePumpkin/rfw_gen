...
All 514 tests move to `rfw_gen_builder/test/`.
...
rfw_gen --> package:rfw (runtime only)
```
...
**golden_test job (ubuntu-latest only):**
```yaml
steps:
  - flutter pub get
...
```yaml
steps:
  - dart pub publish rfw_gen        # first
  - dart pub publish rfw_gen_builder # second, depends on rfw_gen
```
...
1. Package split (rfw_gen ↔ rfw_gen_builder)
2. Update barrel exports and imports
3. Verify all 514 tests pass in new location
4. Add LICENSE (BSD-3-Clause)
5. Update pubspec.yaml metadata
6. Add .pubignore
7. Write README.md (both packages)
8. Write CHANGELOG.md (both packages)
9. Add dartdoc comments
10. Add example/example.dart (rfw_gen)
11. Set up CI/CD (ci.yml + publish.yml)
12. `dart pub publish --dry-run` validation
13. Publish
